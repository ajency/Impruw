 ...
 */
	...
	<p>$message</p>";
        ...
        if (wp_mail($admin_email, $mailsubject, $mailbody))
            wp_send_json(array('code' => 'OK')); else
 ...
 */
    ...
    function impruw_register_room_init()
    {
        ...
        $labels = array('name' => 'Rooms', 'singular_name' => 'Room', 'add_new' => 'Add New', 'add_new_item' => 'Add New Room', 'edit_item' => 'Edit Room', 'new_item' => 'New Romm', 'all_items' => 'All Rooms', 'view_item' => 'View Romms', 'search_items' => 'Search Rooms', 'not_found' => 'No Rooms found', 'not_found_in_trash' => 'No Rooms found in Trash', 'parent_item_colon' => '', 'menu_name' => 'Rooms');

        $args = array('labels'   => $labels, 'label' => __('room', "impruwclientparent"), 'public' => TRUE, 'publicly_queryable' => TRUE, 'show_ui' => TRUE, 'show_in_menu' => TRUE, 'query_var' => TRUE, 'rewrite' => array('slug' => 'room'), 'capability_type' => 'post', 'has_archive' => TRUE, 'hierarchical' => FALSE, 'menu_position' => null, // 'menu_icon' => '' . $url . '/images/room.png',
 ...
 */
     ...
     */
        ...
        $labels = array('name' => _x('Facilities', 'taxonomy general name'), 'singular_name' => _x('Facility', 'taxonomy singular name'), 'search_items' => __('Search Facilities', 'impruwclientparent'), 'popular_items' => __('Popular Faclilities', 'impruwclientparent'), 'all_items' => __('All Facilities', 'impruwclientparent'), 'parent_item' => null, 'parent_item_colon' => null, 'edit_item' => __('Edit Facility', 'impruwclientparent'), 'update_item' => __('Update Facility', 'impruwclientparent'), 'add_new_item' => __('Add New Facility', 'impruwclientparent'), 'new_item_name' => __('New Facility Name', 'impruwclientparent'), 'separate_items_with_commas' => __('Separate facilities with commas'), 'add_or_remove_items' => __('Add or remove facilities', 'impruwclientparent'), 'choose_from_most_used' => __('Choose from the most used facilities', 'impruwclientparent'), 'not_found' => __('No facilities found.', 'impruwclientparent'), 'menu_name' => __('Facilities', 'impruwclientparent'));

        $args = array('hierarchical' => FALSE, 'labels' => $labels, 'show_ui' => TRUE, 'show_admin_column' => TRUE, 'update_count_callback' => '_update_post_term_count', 'query_var' => TRUE, 'rewrite' => array('slug' => 'facility'));
    ...
    function get_page_markup_JSON($page_id = 0)
    {
        ...
        $data = array('id' => $page_id, 'header' => $d ['header'], 'page' => $d ['page'], 'footer' => $d ['footer']);
    ...
    function get_theme_CSS()
    {
        ...
        if (file_exists($theme_path) && is_dir($theme_path)) {
            ...
            foreach ($css_files as $key => $value) {
                ...
                if (!in_array($value, array(".", "..", "theme-style.css"))) {
    ...
    function save_json_structure()
    {
        ...
        $wpdb->update($wpdb->base_prefix . 'page_layouts', array('title' => 'home-2', 'json' => maybe_serialize($json)), array('id' => 4));
    ...
    function add_new_room($blog_id, $array, $tariff_array)
    {
        ...
        $my_post = array('post_title' => $array ['post_title'], 'post_content' => $array ['post_content'], 'post_status' => 'publish', 'post_author' => $array ['user_id'], 'post_type' => 'impruw_room');
    ...
    function add_room_tariff($post_id, $tariff_array)
    {
        ...
        foreach ($tariff_array as $tariff) {
            ...
            $wpdb->insert($wpdb->prefix . 'room_tariffs', array('start_date' => $start_date, 'end_date' => $end_date, 'post_id' => $post_id, 'attributes' => $attributes, 'add_ons' => $add_ons));
        }
    }
    ...
    function get_content_markup()
    {
        ...
        if ($data)
            echo json_encode(array('code' => 'OK', 'data' => $data)); else
 ...
 */
    ...
    function get_menu_to_array($mn, $by = 'name')
    {
        ...
        foreach ((array)$m as $menu_item) {
            ...
            $mn = array('ID' => $menu_item->ID, 'order' => $menu_item->menu_order, 'menu_item_title' => $menu_item->title, 'menu_item_url' => $menu_item->url, 'menu_id' => $menu->term_id);
        ...
        foreach ((array)$m as $menu_item) {
            ...
            $mn = array('ID' => $menu_item->ID, 'order' => $menu_item->menu_order, 'menu_item_title' => $menu_item->title, 'menu_item_url' => $menu_item->url, 'menu_id' => (int)$menu->term_id);
        ...
        $wp_menu = array('id' => (int)$menu->term_id, 'menu_name' => $menu->name, 'menu_slug' => $menu->slug, 'menu_description' => $menu->description, 'menu_items' => $sorted_menu_items);
    ...
    function get_site_menus()
    {
        ...
        if (is_array($menus) && count($menus) > 0) {
            ...
            foreach ($menus as $menu) {
                ...
                foreach ((array)$m as $menu_item) {
                    $mn = array('ID' => $menu_item->ID, 'menuOrder' => $menu_item->menu_order, 'title' => $menu_item->title, 'url' => $menu_item->url);
                ...
                $wp_menus [] = array('id' => (int)$menu->term_id, 'name' => $menu->name, 'slug' => $menu->slug, 'description' => $menu->description, 'items' => $sorted_menu_items);
            }
        }
    ...
    function save_menu_item()
    {
        ...
        if ((int)$item_id === 0) {
            ...
            $item = wp_update_nav_menu_item($menu_id, 0, array('menu-item-title' => $item_title, 'menu-item-classes' => sanitize_title($item_title), 'menu-item-url' => $item_url, 'menu-item-type' => 'custom', 'menu-item-status' => 'publish'));
            if (is_wp_error($item))
                wp_send_json(array('code' => 'ERROR', 'message' => $item->get_error_message())); else
                wp_send_json(array('code' => 'OK', 'itemID' => $item));
        } else {

            wp_update_post(array('ID' => $item_id, 'post_title' => $item_title));
    ...
    function remove_menu_item()
    {
        ...
        if (is_nav_menu_item($item_id) && wp_delete_post($item_id, TRUE))
            wp_send_json(array('code' => 'OK', 'itemID' => $item_id)); else
    ...
    function fetch_all_room_facilities()
    {
        ...
        $room_data = array('facilities' => $room_facilities, 'addontypes' => $addons_types, 'taxtypes' => $tax_types, 'checkinformat' => ($checkin_format == FALSE ? '' : $checkin_format), 'checkintime' => ($checkin_time == FALSE ? '' : $checkin_time), 'additionalpolicies' => ($additional_policies == FALSE ? '' : $additional_policies), 'taxoption' => ($tax_option == FALSE ? '' : $tax_option), 'dateranges' => $date_range);
    ...
    function fetch_all_daterange()
    {
        ...
        if (count($result) > 0) {
            foreach ($result as $daterange_val) {
                ...
                $daterangeplans [] = array('id' => $daterange_val->id, 'from' => $daterange_from, 'to' => $daterange_to, 'from_calendar' => date('d F, Y', strtotime($daterange_val->from_date)), 'to_calendar' => date('d F, Y', strtotime($daterange_val->to_date)), 'label' => $daterange_label, 'plans' => $plans);
            }
    ...
    function fetch_daterange_plans($daterange_id)
    {
        ...
        foreach ($plans_data as $plan_data) {
            ...
            $plans [] = array('plan_id' => $plan_id, 'plan_name' => $plan_name, 'plan_description' => $plan_description, 'weekend_tariff' => $weekend_tariff, 'weekday_tariff' => $weekday_tariff);
        }
    ...
    function fetch_daterange_plan_tariff_data_ajx()
    {
        ...
        if ($result)
            wp_send_json(array('code' => 'OK', 'dateTariff' => $result)); else
    ...
    function save_updated_plan_tariff_ajx()
    {
        ...
        $weekendtariff = array('tariff' => $weekend_tariff, 'maxadults' => $weekend_maxadults, 'maxchildren' => $weekend_maxchildren, 'extra_adult_charges' => $weekend_charges_extra_adult, 'extra_child_charges' => $weekend_charges_extra_child);

        $weekdaytariff = array('tariff' => $weekday_tariff, 'maxadults' => $weekday_maxadults, 'maxchildren' => $weekday_maxchildren, 'extra_adult_charges' => $weekday_charges_extra_adult, 'extra_child_charges' => $weekday_charges_extra_child);
        ...
        if (!empty($daterange_plan_tariff_id)) {
            ...
            if ($tariff_result === FALSE) {
                ...
                $tariff_details = array('weekdaytariff' => $weekday_tariff, 'weekendtariff' => $weekend_tariff, 'daterangePlanTariffId' => $daterange_plan_tariff_id, 'daterangeid' => $daterange_id, 'planid' => $plan_id);
        ...
        if ($addon_types) {
            foreach ($addon_types as $addontype_key => $addontype_val) {
                ...
                if ($addontype_val ['id'] != $addontype_edit)
                    $updated_addon_types [] = array('id' => $addontype_val ['id'], 'label' => $addontype_val ['label'], 'price' => $addontype_val ['price']); else
        ...
        if ($tax_types) {
            foreach ($tax_types as $taxtype_key => $taxtype_val) {
                ...
                if ($taxtype_val ['id'] != $taxtype_edit)
                    $updated_tax_types [] = array('id' => $taxtype_val ['id'], 'name' => $taxtype_val ['name'], 'percent' => $taxtype_val ['percent']); else
    ...
    function update_room_facility()
    {
        ...
        $facility_data = wp_update_term($facility_id, 'impruw_room_facility', array('name' => $facility_name, 'slug' => $facility_slug));
    ...
    function update_checkintime()
    {
        ...
        if ($result_checkinformat == TRUE || $result_checkintime == TRUE)
            wp_send_json(array('code' => 'OK', 'msg' => _('Check-in time updated successfully'), 'checkinTime' => $checkintime, 'checkinformat' => $checkin_format)); else
 ...
 */
    function update_taxoption()
    {
        ...
        if ($result)
            wp_send_json(array('code' => 'OK', 'msg' => _('Tax option updated successfully'), 'taxoption' => $tax_option)); else
    ...
    function update_checkinformat()
    {
        ...
        if ($result)
            wp_send_json(array('code' => 'OK', 'msg' => _('Check-in format updated successfully'), 'checkinformat' => $checkin_format)); else
     ...
     */
        ...
        $result = $wpdb->insert($table_name, array('from_date' => $from_daterange, 'to_date' => $to_daterange, 'label' => $label), array('%s', '%s', '%s'));

        $datarange_data = array('id' => $wpdb->insert_id, 'from_date' => date('d/m/y', strtotime($_POST ['fromdaterange'])), 'to_date' => date('d/m/y', strtotime($_POST ['todaterange'])), 'label' => $label);
        switch_to_blog(get_current_blog_id());

        if ($result == TRUE)
            wp_send_json(array('code' => 'OK', 'msg' => _('Date range is successfully added'), 'daterange' => $datarange_data)); else
     ...
     */
        ...
        $daterange_data = array('from_date' => date('d/m/y', strtotime($_POST ['from_daterange'])), 'to_date' => date('d/m/y', strtotime($_POST ['to_daterange'])));

        if ($result == TRUE)
            wp_send_json(array('code' => 'OK', 'msg' => _('Date range updated successfully'), 'daterange_data' => $daterange_data)); else
    ...
    function add_new_plan_tariff()
    {
        ...
        if (!empty($plan_id)) {
            ...
            if ($tariff_result === FALSE) {
                ...
                $tariff_details = array('planid' => $plan_id, 'daterangeid' => $daterange_id, 'plantariffid' => $tariff_result, 'weekdaytariff' => $weekday_tariff, 'weekendtariff' => $weekend_tariff, 'daterangePlanTariffId' => $tariff_result);
    ...
    function add_new_plan_ajx()
    {
        ...
        if ($plan_result !== FALSE) {
            ...
            $plan_details = array('plan' => $plantype, 'plandescription' => $plandescription, 'planid' => $plan_result);
    ...
    function update_plan_ajx()
    {
        ...
        $updated_plan = array('plan_id' => $plan_data ['hdn_planid'], 'plan_label' => $plan_data ['plantype'], 'plan_description' => $plan_data ['plandescription'], 'daterange_id' => $plan_data ['hdn_daterange']);
        ...
        if (!$result)
            wp_send_json(array('code' => 'ERROR', 'msg' => _('Error updating plan'), 'plandata' => $updated_plan)); else
            ...
            if (count($plans) > 0) {
                ...
                foreach ($plans as $plan_key => $plan_val) {
                    ...
                    if ($plan_val ['id'] == $plan_id) {
                        $updated_plan_data [] = array('id' => trim($plan_id), 'label' => trim($plantype), 'description' => trim($plandescription));
                    } else {
                        $updated_plan_data [] = array('id' => trim($plan_val ['id']), 'label' => trim($plan_val ['label']), 'description' => trim($plan_val ['description']));
                    }
                }
    ...
    function update_daterangeplan_tariff($weekend_tariff, $weekday_tariff, $daterangeplan_id)
    {
        ...
        $result = $wpdb->update($table_name, array('tarriff' => maybe_serialize(array('weekend' => $weekend_tariff, 'weekday' => $weekday_tariff))), array('id' => $daterangeplan_id), array('%s'), array('%d'));
 ...
 */
    ...
    function add_daterangeplan_tariff($weekend_tariff, $weekday_tariff, $plan_id, $daterange_id)
    {
        ...
        $result = $wpdb->insert($table_name, array('daterange_id' => $daterange_id, 'plan_id' => $plan_id, 'tarriff' => maybe_serialize(array('weekend' => $weekend_tariff, 'weekday' => $weekday_tariff))), array('%s', '%s', '%s'));
        ...
        if (count($plan_details1)) {
            foreach ($plan_details1 as $plan_data1) {
                ...
                if ($plan_data1 ['id'] != $plan_id) {
                    $new_plan_data1 [] = array('id' => $plan_data1 ['id'], 'label' => $plan_data1 ['label'], 'description' => $plan_data1 ['description']);
                }
            }
        }
    ...
    {
        switch_to_blog($blog_id);
        $my_post = array('ID' => $array ['ID'], 'post_title' => $array ['post_title'], 'post_content' => $array ['post_content'], 'post_status' => 'publish', 'post_author' => $array ['user_id'], 'post_type' => 'impruw_room');
        // print_r($array['terms']);exit;
        // Insert the post into the database
 ...
 */
    ...
    function delete_room_ajx()
    {
        ...
        if (!$result)
            wp_send_json(array('code' => 'ERROR', 'data' => 'Error deleting room')); else
    ...
    function save_initial_layout()
    {
        ...
        if (is_array($header))
            update_option('theme-header', $header); else
            delete_option('theme-header');

        if (is_array($pagejson))
            update_post_meta($page_id, 'page-json', $pagejson); else
            delete_post_meta($page_id, 'page-json');

        if (is_array($footer))
            update_option('theme-footer', $footer); else
    ...
    function get_element_markup()
    {
        ...
        wp_send_json(array('code' => 'OK', 'html' => $html));
    ...
    function get_image_url()
    {
        ...
        if ($path === FALSE)
            wp_send_json(array('code' => 'ERROR')); else
    ...
    function get_rooms()
    {
        ...
        while ($rooms->have_posts()) :
            ...
            $r [] = array('id' => get_the_ID(), 'name' => get_the_title());
        endwhile;
    ...
    function impruw_app_model()
    {
        if (!is_user_logged_in())
            return '{}'; else
            return json_encode(array('userId' => get_current_user_id(), 'accessToken' => 'oicdaso34983hsvdsklkccxiodas897', 'loginStatus' => TRUE));
    }
    ...
    function switch_theme_colour()
    {
        ...
        $colours = array('primary1' => 'red', 'secondary1' => '#29333E', 'tertiary1' => '#F2F2F2');
  ...
  wp_enqueue_script ('maps', get_template_directory_uri () . 'js/contact.js');
    ...
    $base_element_templates = array('Row' => array(array('name' => 'Center Container'), array('name' => 'Grey Background'), array('name' => 'Footer Container'), array('name' => 'Column Dividers')), 'Menu' => array(array('name' => 'Slimmenu'), array('name' => 'Footer Menu')), 'Title' => array(array('name' => 'Sub Title')), 'Image' => array(array('name' => 'Image Bordered'), array('name' => 'Image Rounded')), 'Address' => array(array('name' => 'Default Style', 'template' => '<ul><li><span class="fui-home"></span> {{street}}, {{city}}, {{postal_code}}, {{country}}</li><li><span class="glyphicon glyphicon-earphone"></span> {{phone_no}}</li><li><span class="fui-mail"></span> {{email}}</li></ul>'), array('name' => 'Small Address', 'template' => '<div><div class="info"> {{street}}, {{city}}, {{postal_code}}, {{country}}</div><div class="info"> {{phone_no}}</div><div class="info"> {{email}}</div></div>')), 'Social' => array(array('name' => 'Social Right'), array('name' => 'Social Center'), array('name' => 'Social Left')), 'RoomSummary' => array(array('name' => 'Room Summary Default', 'template' => '<div class="room-img"><img src="{{image_url}}" width="100%" class="img-responsive"></div><div class="room-title">{{post_title}}</div><div class="room-excerpt">{{post_content}}</div><div class="room-actions"><div class="price">Total: {{no_of_rooms}}<small> rooms</small></div><button class="btn btn-room">View Details</button></div>'), array('name' => 'Room Summary New', 'template' => 'Suraj<div class="room-img"><img src="{{image_url}}" width="100%"  class="img-responsive"></div><div class="room-title">{{post_title}}</div><div class="room-excerpt">{{post_content}}</div><div class="room-actions"><div class="price">Total: {{no_of_rooms}}<small> rooms</small></div><button class="btn btn-room">View Details</button></div>')));
