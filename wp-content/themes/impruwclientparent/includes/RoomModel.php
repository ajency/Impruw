<?php
    ...
    class RoomModel
    {
        ...
        function min_max_weekday_weekend_tariff($daterange_tariff = array())
        {
            ...
            $tariff = array('max_weekend' => '', 'min_weekend' => '', 'max_weekday' => '', 'min_weekday' => '');
        ...
        function get_all_roomdata($room_retrival_data = array())
        {
            ...
            if (is_array($room_attachments)) {
                foreach ($room_attachments as $attachment) {
                    if ($attachment != '') {
                        $attachment_data[] = array('attach_id' => $attachment, 'attach_url' => wp_get_attachment_thumb_url($attachment));
                    }
            ...
            $rooom_data = array('roomType' => $this->get('post_title'), 'roomShortDesc' => $this->get('post_excerpt'), 'roomDesc' => $this->get('post_content'), 'inventory' => get_post_meta($this->get('ID'), 'inventory', TRUE), 'id' => (int)$this->get('ID'), 'checkintime' => get_option('checkin-time'), 'checkinformat' => get_option('checkin-format'), 'daterangetariff' => $daterange_tariff, 'facilities' => $this->get_room_facilities(), 'roomAttachments' => $attachment_data, 'roomFeaturedImg' => $this->get_featured_img(), 'minmaxTariff' => $min_max_tariff);
        ...
        function get_featured_img()
        {
            ...
            $featured_image = array('id' => $featured_img_id, 'url' => $image_url[0]);
        ...
        function get_room_tariffs($tariffplan_options = array())
        {
            ...
            if ($qry_result) {
                foreach ($qry_result as $plan__tariff) {
                    ...
                    $daterange_tariff[] = array('daterangePlanTariffId' => $plan__tariff->dateplan_tariffid, 'daterange_id' => $plan__tariff->daterange_id, 'planId' => $plan__tariff->plan_id, 'fromDate' => date('d/m/Y', strtotime($plan__tariff->from_date)), 'toDate' => date('d/m/Y', strtotime($plan__tariff->to_date)), 'weekdayTariff' => $weekday_tariff, 'weekEndTariff' => $weekend_tariff, 'planName' => ucwords($plan_name), 'qry' => $qry_tariff
