<td>
	<div class="room-details">
		<h4>Some room title here</h4>
		<p class="desc">Creates and returns a new debounced version of the passed function which will postpone its execution until after wait milliseconds have elapsed...</p>
		<div class="row info-strip">
			<div class="col-md-4 strip">
				<span>10</span> Rooms
			</div>
			<div class="col-md-4 strip">
				<span>12Hr</span> Hour Format
			</div>
			<div class="col-md-4 strip">
				<span>9:00AM</span>
			</div>
		</div>
	</div>
</td>
<td>
	<div class="tariff">
		<div class="row tariff-plan">
			<div class="col-sm-6 plan">
				<h6>Plan Name</h6>
				1st Jan to 31st Mar
			</div>
			<div class="col-sm-3 weekday">
				Weekday
				<div class="price">$200</div>
			</div>
			<div class="col-sm-3 weekend">
				Weekend
				<div class="price">$300</div>
			</div>
		</div>
	</div>
</td>
<td class="actions">
	<a href="#edit-room/20" class="btn btn-xs editroom_link">
	<span class="glyphicon glyphicon-pencil"></span> Edit</a> 
	<a href="#delete-room/20" class="btn btn-danger btn-xs deleteroom_link">
	<span class="glyphicon glyphicon-trash"></span> Delete</a>
</td>