<?php
    ...
    function wp_impruw_create_user($user_data_array, $user_default_language)
    {
        ...
        $wpdb->update($wpdb->prefix . 'users', array('user_activation_key' => $activation_key, 'user_status' => 2), array('ID' => $user_id));
        ...
        if (is_user_logged_in())
            $initiator_id = get_current_user_id(); else
    ...
    function create_new_site($blog_id, $blog_name, $blog_title, $user_id, $file_name, $user_default_language, $data)
    {
        ...
        $pages = array('Dashboard' => array('content' => 'Dashboard Content', 'template' => 'new-dashboard.php'), 'Home' => array('content' => 'Home Content', 'template' => ''), 'About Us' => array('content' => 'About Content', 'template' => ''), 'Contact Us' => array('content' => 'Contact Content', 'template' => ''), 'Site Builder' => array('content' => 'Site Builder Content', 'template' => 'new-builder.php'), 'Single Room' => array('content' => 'Single Room', 'template' => ''), 'Rooms' => array('content' => 'Rooms', 'template' => ''));

        //start creating initial pages
        foreach ($pages as $key => $value) {

            $post_id = add_new_post_to_blog($new_blog_id, $user_id, $key, $value['content'], 'page', $value['template']);
    ...
    function add_new_post_to_blog($blog_id, $user_id, $post_title, $post_content, $post_type, $post_template)
    {
        ...
        $my_post = array('post_title' => $post_title, 'post_content' => $post_content, 'post_status' => 'publish', 'post_author' => $user_id, 'post_type' => $post_type);
    ...
    function add_menu_to_blog($user_id, $blog_id)
    {
        ...
        if (!$run_once) {
            ...
            foreach (get_all_menu_pages() as $page):
                //then add the actuall link/ menu item and you do this for each item you want to add
                wp_update_nav_menu_item($menu_id_footer, 0, array('menu-item-title' => $page->post_title, 'menu-item-classes' => $page->post_name, 'menu-item-url' => get_permalink($page->ID), 'menu-item-status' => 'publish'));
