<?php
    ...
    function add_to_page_layouts()
    {
        ...
        if (count($layout_data) == 0) {
            $wpdb->insert($wpdb->prefix . 'page_layouts', array('title' => $title, 'type' => $type, 'path' => $path));
        }
    }
    ...
    function add_term_to_facility()
    {
        ...
        foreach ($blogs as $blog) {
            ...
            $term_id = wp_insert_term('Wifi Available', 'impruw_room_facility', array('description' => 'Wifi Available', 'slug' => $slug, 'parent' => 0,));

            //add term Swimming pool  taxonomy Facility
            $slug    = sanitize_title('swimming_pool');
            $term_id = wp_insert_term('Swimming Pool', 'impruw_room_facility', array('description' => 'Swimming Pool', 'slug' => $slug, 'parent' => 0,));
